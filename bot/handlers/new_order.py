...
from db.queries import find_or_create_client, create_order, get_all_masters, get_order_by_id
...
async def got_client_name(msg: Message, state: FSMContext):
    ...
    await msg.answer("📞 Введите номер телефона клиента (или - если неизвестен):")
...
async def got_client_phone(msg: Message, state: FSMContext):
    ...
    await msg.answer("📱 Выберите тип устройства:", reply_markup=device_type_keyboard())
...
async def got_device_type(cb: CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("📋 Введите модель устройства (iPhone 14, Galaxy S23, MacBook Pro и т.д.):")
...
async def got_device_model(msg: Message, state: FSMContext):
    ...
    await msg.answer("🔧 Опишите неисправность:")
...
async def got_price(msg: Message, state: FSMContext):
    ...
    await msg.answer("🚨 Выберите приоритет:", reply_markup=priority_keyboard())
...
async def _create_order_final(msg, state: FSMContext):
    ...
    await state.clear()
    await msg.answer(f"✅ Заказ создан!\n\n{fmt_order_card(order_full)}")
