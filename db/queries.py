...
def find_or_create_client(name: str, phone: str) -> dict:
    sb = get_client()
...
def update_order_status(order_id: str, new_status: str, changed_by: str, comment: str = "") -> dict:
    sb = get_client()
...
def update_order_field(order_id: str, field: str, value) -> dict:
    sb = get_client()
    sb.table("orders").update({field: value}).eq("id", order_id).execute()
    return get_order_by_id(order_id)


def assign_master(order_id: str, master_id: str) -> dict:
    sb = get_client()
...
def add_part(order_id: str, name: str, cost: int) -> dict:
    sb = get_client()
...
def get_parts_for_order(order_id: str) -> list:
    sb = get_client()
    res = sb.table("parts").select("*").eq("order_id", order_id).execute()
    return res.data


def update_part_status(part_id: str, new_status: str) -> dict:
    sb = get_client()
...
def add_cash_entry(user_id: str, type_: str, amount: int, description: str = "", order_id: str | None = None) -> dict:
    sb = get_client()
...
def get_master_stats(master_id: str, days: int = 30) -> dict:
    sb = get_client()
    from datetime import datetime, timedelta
    since = (datetime.utcnow() - timedelta(days=days)).isoformat()
    ...
    total_revenue = sum(o["price"] for o in done)
    return {
        "total": len(orders),
        "done": len(done),
...
def get_orders_stats(days: int = 30) -> dict:
    sb = get_client()
    from datetime import datetime, timedelta
    since = (datetime.utcnow() - timedelta(days=days)).isoformat()
